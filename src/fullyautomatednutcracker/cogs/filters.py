...
class Filters(commands.Cog):
    ...
    @commands.command(aliases=["biflag", "bisexual"])
    async def bi(self, ctx):
        ...
        # Open the filter
        bi_filter = Image.open('content/filters/Bi_flag_overlay.png')
        resized_filter = None
        ...
        for idx, frame in enumerate(download.frames):
            ...
            download.frames[idx] = self.do_arg_resize(frame, arguments)
            if not resized_filter:
                resized_filter = common_imaging.resize(bi_filter, download.frames[idx].size)
            # Paste the filter onto the frame
            download.frames[idx].paste(resized_filter, (0, 0), resized_filter)

        # Ship it
        if download.is_gif:
            await self.save_gif_and_send(arguments, ctx.author, ctx.channel, download.frames, file_name="Bimask", things_to_close=(download.image, bi_filter, resized_filter))
        else:
            await self.save_img_and_send(arguments, ctx.author, ctx.channel, download.frames[0], file_name="Bimask", things_to_close=(download.image, bi_filter, resized_filter))
        del download
    ...
    async def asexual(self, ctx):
        ...
        # Open the filter
        asexual_filter = Image.open('content/filters/asexual_flag_overlay.png')
        resized_filter = None
        ...
        for idx, frame in enumerate(download.frames):
            ...
            download.frames[idx] = self.do_arg_resize(frame, arguments)
            if not resized_filter:
                resized_filter = common_imaging.resize(asexual_filter, download.frames[idx].size)
            # Paste the filter onto the frame
            download.frames[idx].paste(resized_filter, (0, 0), resized_filter)

        # Ship it
        if download.is_gif:
            await self.save_gif_and_send(arguments, ctx.author, ctx.channel, download.frames, file_name="Asexual Mask", things_to_close=(download.image, asexual_filter, resized_filter))
        else:
            await self.save_img_and_send(arguments, ctx.author, ctx.channel, download.frames[0], file_name="Asexual Mask", things_to_close=(download.image, asexual_filter, resized_filter))
        del download

    @commands.command(aliases=["nonbinary", "nonb"])
    async def enby(self, ctx):
        ...
        # Open the filter
        enby_filter = Image.open('content/filters/enby_flag_overlay.png')
        resized_filter = None
        ...
        for idx, frame in enumerate(download.frames):
            ...
            download.frames[idx] = self.do_arg_resize(frame, arguments)
            if not resized_filter:
                resized_filter = common_imaging.resize(enby_filter, download.frames[idx].size)
            # Paste the filter onto the frame
            download.frames[idx].paste(resized_filter, (0, 0), resized_filter)

        # Ship it
        if download.is_gif:
            await self.save_gif_and_send(arguments, ctx.author, ctx.channel, download.frames, file_name="Enby Mask", things_to_close=(download.image, enby_filter, resized_filter))
        else:
            await self.save_img_and_send(arguments, ctx.author, ctx.channel, download.frames[0], file_name="Enby Mask", things_to_close=(download.image, enby_filter, resized_filter))
        del download

    @commands.command(aliases=["gf", "genfluid"])
    async def gender_fluid(self, ctx):
        ...
        # Open the filter
        gender_fluid_filter = Image.open('content/filters/gender_fluid_flag_overlay.png')
        resized_filter = None
        ...
        for idx, frame in enumerate(download.frames):
            ...
            download.frames[idx] = self.do_arg_resize(frame, arguments)
            if not resized_filter:
                resized_filter = common_imaging.resize(gender_fluid_filter, download.frames[idx].size)
            # Paste the filter onto the frame
            download.frames[idx].paste(resized_filter, (0, 0), resized_filter)

        # Ship it
        if download.is_gif:
            await self.save_gif_and_send(arguments, ctx.author, ctx.channel, download.frames, file_name="Gender Fluid Mask", things_to_close=(download.image, gender_fluid_filter, resized_filter))
        else:
            await self.save_img_and_send(arguments, ctx.author, ctx.channel, download.frames[0], file_name="Gender Fluid Mask", things_to_close=(download.image, gender_fluid_filter, resized_filter))
        del download

    @commands.command(aliases=["gq", "genqueer"])
    async def gender_queer(self, ctx):
        ...
        # Open the filter
        gender_queer_filter = Image.open('content/filters/gender_queer_flag_overlay.png')
        resized_filter = None
        ...
        for idx, frame in enumerate(download.frames):
            ...
            download.frames[idx] = self.do_arg_resize(frame, arguments)
            if not resized_filter:
                resized_filter = common_imaging.resize(gender_queer_filter, download.frames[idx].size)
            # Paste the filter onto the frame
            download.frames[idx].paste(resized_filter, (0, 0), resized_filter)

        # Ship it
        if download.is_gif:
            await self.save_gif_and_send(arguments, ctx.author, ctx.channel, download.frames, file_name="gaygaygay", things_to_close=(download.image, gender_queer_filter, resized_filter))
        else:
            await self.save_img_and_send(arguments, ctx.author, ctx.channel, download.frames[0], file_name="gaygaygay", things_to_close=(download.image, gender_queer_filter, resized_filter))
        del download
    ...
    async def lesbian(self, ctx):
        ...
        # Open the filter
        lesbian_filter = Image.open('content/filters/lesbian_flag_overlay.png')
        resized_filter = None
        ...
        for idx, frame in enumerate(download.frames):
            ...
            download.frames[idx] = self.do_arg_resize(frame, arguments)
            if not resized_filter:
                resized_filter = common_imaging.resize(lesbian_filter, download.frames[idx].size)
            # Paste the filter onto the frame
            download.frames[idx].paste(resized_filter, (0, 0), resized_filter)

        # Ship it
        if download.is_gif:
            await self.save_gif_and_send(arguments, ctx.author, ctx.channel, download.frames, file_name="Lesbian Mask", things_to_close=(download.image, lesbian_filter, resized_filter))
        else:
            await self.save_img_and_send(arguments, ctx.author, ctx.channel, download.frames[0], file_name="Lesbian Mask", things_to_close=(download.image, lesbian_filter, resized_filter))
        del download
    ...
    async def pan(self, ctx):
        ...
        # Open the filter
        pan_filter = Image.open('content/filters/pan_flag_overlay.png')
        resized_filter = None
        ...
        for idx, frame in enumerate(download.frames):
            ...
            download.frames[idx] = self.do_arg_resize(frame, arguments)
            if not resized_filter:
                resized_filter = common_imaging.resize(pan_filter, download.frames[idx].size)
            # Paste the filter onto the frame
            download.frames[idx].paste(resized_filter, (0, 0), resized_filter)

        # Ship it
        if download.is_gif:
            await self.save_gif_and_send(arguments, ctx.author, ctx.channel, download.frames, file_name="Pansexual Mask", things_to_close=(download.image, pan_filter, resized_filter))
        else:
            await self.save_img_and_send(arguments, ctx.author, ctx.channel, download.frames[0], file_name="Pansexual Mask", things_to_close=(download.image, pan_filter, resized_filter))
        del download
    ...
    async def polysexual(self, ctx):
        ...
        # Open the filter
        polysexual_filter = Image.open('content/filters/polysexual_flag_overlay.png')
        resized_filter = None
        ...
        for idx, frame in enumerate(download.frames):
            ...
            download.frames[idx] = self.do_arg_resize(frame, arguments)
            if not resized_filter:
                resized_filter = common_imaging.resize(polysexual_filter, download.frames[idx].size)
            # Paste the filter onto the frame
            download.frames[idx].paste(resized_filter, (0, 0), resized_filter)

        # Ship it
        if download.is_gif:
            await self.save_gif_and_send(arguments, ctx.author, ctx.channel, download.frames, file_name="Polysexual Mask", things_to_close=(download.image, polysexual_filter, resized_filter))
        else:
            await self.save_img_and_send(arguments, ctx.author, ctx.channel, download.frames[0], file_name="Polysexual mask", things_to_close=(download.image, polysexual_filter, resized_filter))
        del download
